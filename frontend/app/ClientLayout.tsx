...
const inter = Inter({
  variable: "--font-inter", // Puedes cambiar el nombre de la variable CSS si lo deseas
  subsets: ["latin"],
})

// Las fuentes Geist y Geist_Mono ya no se usan aquí.
// Si quieres usarlas, necesitarías descargar los archivos de la fuente
// y configurarlos con `next/font/local`.
      ...
      <body className={`${inter.variable} antialiased bg-[#fafafa] text-black`}>
        ...
        <div className="flex">
          ...
          >
                      ...
                      <span className="text-black">
                        ...
                        {link.icon === "svg-proveedores" && (
                          ...
                          >
                            ...
                            <path
                              d="M16 13V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h1"
                              stroke="currentColor"
                              strokeWidth="2"
                            />
                        ...
                        {link.icon !== "svg-categorias" && link.icon !== "svg-proveedores" && (
                          <span className={`material-icons ${sidebarOpen ? "text-lg" : "text-base"} text-black`}>
                            {link.icon}
                          </span>
                        )}
                      </span>
            ...
            {/* Botón de Configuración (fijo en la parte inferior) */}
            <div className="mt-auto w-full px-2 pb-2">
              ...
              {!sidebarOpen && (
                <span className="absolute left-full bottom-2 ml-2 px-2 py-1 rounded bg-black text-white text-xs opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 transition-opacity duration-200">
                  Configuración
                </span>
              )}
            </div>
          ...
          {mobileSidebar && (
            <div className="fixed inset-0 z-50 flex">
              <aside className="w-60 bg-white border-r border-[#ececec] flex flex-col py-8 px-3 shadow-sm h-full">
                ...
                {/* Botón de Configuración para móvil */}
                <div className="mt-auto w-full px-2 pb-2">
                  <Link
                    href="/configuracion"
                    className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#f3f4f6] transition text-base font-medium text-black"
          ...
          >
            ...
            {/*main content */}
            <main className="flex-1 p-8 bg-[#fafafa] transition-all duration-200" style={{ marginTop: 64 }}>
              {children}
            </main>
          </div>
        </div>
